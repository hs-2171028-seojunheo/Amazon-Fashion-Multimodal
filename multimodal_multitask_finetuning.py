...
# ==========================================
# 3. 모델 아키텍처 (L04~L08 전체 적용 + Multi-Task)
# ==========================================
class ModalityDropout(nn.Module):
    def __init__(self, p=0.3): # 텍스트/이미지를 더 쓰게 만들기 위해 Dropout 확률을 30%로 증가 (L07 적용)
        super().__init__()
        self.p = p
...
class MultitaskFashionModel(nn.Module):
    def __init__(self, num_cat, hidden_dim=256):
        ...
        self.modality_dropout = ModalityDropout(0.3)
        self.gmu = ThreeWayGMU(hidden_dim)
        
        # [핵심 변경점] 모달리티별 독립적인 예측 레이어 (쪽지 시험용)
    ...
    def forward(self, ids, mask, pixels, price, miss, cat):
        ...
        # 2. 쪽지 시험 (독립 예측) - 모델이 이미지/텍스트 특징을 똑바로 추출했는지 검사
...
def train_epoch(model, loader, optimizer, scheduler, device, acc_steps, epoch, phase_name):
    ...
    for i, batch in enumerate(pbar):
        ...
        # [핵심 변경점] 3개의 Loss를 합산 (이미지와 텍스트 강제 학습)
        ...
        # 이미지와 텍스트에 가중치 0.5씩 부여하여 페널티 부과
...
def evaluate(model, loader, device, epoch, phase_name):
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=f"[{phase_name}] Epoch {epoch} Eval"):
            ...
            # 평가할 때는 오직 최종 융합 점수(out_fused)만 기준으로 평가
...
def main():
    ...
    for epoch in range(1, EPOCHS_PHASE2 + 1):
        ...
        if val_mae < best_mae:
            ...
            print(f"New Best Multitask Model Saved (MAE: {best_mae:.4f})\n")
        else:
            print()
